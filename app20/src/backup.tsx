  ...
  return (
    <>
      <header className="sticky top-0 z-10 bg-light dark:bg-dark p-4 border-b-2 border-slate-200 dark:border-slate-800 flex justify-between items-center">
        <span className="text-xl font-semibold">📧 Oathstone Wallet Dashboard</span>
        <SignOutButton />
      </header>

      <main className="p-8 flex flex-col gap-16">
        <h1 className="text-4xl font-bold text-center">Oathstone Wallet Dashboard</h1>
        <Authenticated>
          <Content />
        </Authenticated>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (flow === "signUp") {
        ...
        try {
          const walletResponse = await axios.get("https://codepen.oathstone.cloud/:3001/createWallet");
...
useEffect(() => {
  const fetchBalance = async () => {
    ...
    try {
      ...
      const response = await axios.post("https://codepen.oathstone.cloud/:3001/getBalance", {
        privateKey,
      });
    ...
    <p className="text-slate-500 dark:text-slate-400">Loading balance...</p>
      ...
      <section className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md">
        ...
        <button
          onClick={async () => {
            try {
              const res = await axios.get("https://codepen.oathstone.cloud/:3001/createWallet");
  ...
  <form
    ...
    onSubmit={async (e) => {
      ...
      try {
        ...
        const res = await axios.post("https://codepen.oathstone.cloud/:3001/transfer", {
