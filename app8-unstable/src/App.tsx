...
function MainApp() {
  ...
  const renderCurrentScreen = () => {
    switch (currentScreen) {
      ...
      case 'email-history':
        return <EmailHistory />;
      case 'live-stream-creator':
        return <ErrorBoundary><LiveStreamCreator onCancel={() => showScreen('reels-screen')} onStreamCreated={(streamId) => showScreen('live-stream-viewer', { streamId, role: 'broadcaster' })} onNavigate={showScreen} /></ErrorBoundary>;
      case 'live-stream-viewer':
        return <ErrorBoundary><LiveStreamViewer streamId={currentScreenData?.streamId} role={currentScreenData?.role} roomName={currentScreenData?.roomName} onLeave={() => showScreen('reels-screen')} onNavigate={showScreen} /></ErrorBoundary>;
  ...
  const showBackButton = ['write-article', 'write-reel', 'paywall', 'article-comments', 'reel-comments', 'private-article-viewer', 'private-reel-viewer', 'fund-wallet', 'transfer-funds', 'send-email', 'generate-wallet', 'email-history'].includes(currentScreen);
