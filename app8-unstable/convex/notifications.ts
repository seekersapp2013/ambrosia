...
export const NOTIFICATION_TYPES: Record<string, NotificationType> = {
  ...
  LIVE_STREAM_STARTED: {
    id: 'LIVE_STREAM_STARTED',
    name: 'Live Stream Started',
    description: 'Someone you follow started a live stream',
    category: 'content',
    priority: 'high',
    batchable: false,
    defaultChannels: ['in_app', 'email']
...
async function generateNotificationContent(
  ...
  switch (notificationType.id) {
    ...
    case 'LIVE_STREAM_STARTED':
      const streamTitle = await getContentTitle(ctx, relatedContentType, relatedContentId);
      return {
        title: '🔴 Live Now!',
        message: `${actorName} is live now: "${streamTitle}"`
      };
...
async function getContentTitle(ctx: any, contentType?: string, contentId?: string): Promise<string> {
  ...
  try {
    if (contentType === 'article') {
      const article = await ctx.db.get(contentId as Id<"articles">);
      return article?.title || 'article';
    } else if (contentType === 'reel') {
      const reel = await ctx.db.get(contentId as Id<"reels">);
      return reel?.caption || 'reel';
    } else if (contentType === 'liveStream') {
      const stream = await ctx.db.get(contentId as Id<"liveStreams">);
