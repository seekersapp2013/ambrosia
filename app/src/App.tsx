  ...
  return (
    <>
      <header className="sticky top-0 z-10 bg-light dark:bg-dark p-4 border-b-2 border-slate-200 dark:border-slate-800 flex justify-between items-center">
        <span className="text-xl font-semibold">📧 Oathstone Codepen Dashboard</span>
  ...
  <button
    ...
    className={`px-4 py-2 rounded-md ${
      currentView === "dashboard"
        ...
        : "bg-slate-200 dark:bg-slate-700 text-dark dark:text-light"
    }`}
  >
   💰  Dashboard
  </button>
  ...
  >
   ⚒️  Smart Contracts
      ...
      <main className="p-8 flex flex-col gap-16">
        <h1 className="text-4xl font-bold text-center">Oathstone Codepen Dashboard</h1>
        <Authenticated>
...
<section className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md">
  ...
  {balanceError ? (
    ...
    <button
            onClick={() => {
              navigator.clipboard.writeText(balance.celo.address);
            }}
            className="px-3 py-1 bg-slate-600 hover:bg-slate-700 text-white text-sm rounded-md"
          >
            Copy Address
          </button>
    {/* Display the private key from localStorage */}
      {_storedWallet?.wallet?.privateKey && (
        <p><strong>Private Key:</strong> {_storedWallet.wallet.privateKey}</p>
      )}
                <button
            onClick={() => {
              navigator.clipboard.writeText(_storedWallet.wallet.privateKey);
            }}
            className="px-3 py-1 bg-slate-600 hover:bg-slate-700 text-white text-sm rounded-md"
          >
            Copy Praivate Key
   ...
   <p>You can fund your Wallet with Test CELO at https://faucet.celo.org/alfajores <br />
   For USD Token, please reach out at graderng@gmail.com
   </p>
    </div>
  ) : (
