...
export default defineSchema({
  ...
  notifications: defineTable({
    userId: v.id("users"),
    type: v.string(), // 'email_sent' | 'wallet_created' | 'payment_received' | 'follow' | 'like' | 'comment'
    title: v.string(),
    message: v.string(),
    isRead: v.boolean(),
    relatedId: v.optional(v.string()), // Could be emailId, articleId, etc.
    createdAt: v.number()
  }).index("by_user", ["userId"]).index("by_user_unread", ["userId", "isRead"])
});
